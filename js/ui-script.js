...
const krds_mainMenuPC = {
  ...
  resetMainMenu() {
    document.querySelectorAll(".main-menu:not(.sample) .gnb-main-trigger:not(.is-link)").forEach((mainTrigger) => {
      mainTrigger.classList.remove("active");
      mainTrigger.setAttribute("aria-expanded", "false");
    });
    document.querySelectorAll(".main-menu:not(.sample) .gnb-toggle-wrap").forEach((toggleWrap) => {
...
const krds_mainMenuMobile = {
  init() {
    const mobileGnb = document.querySelector(".main-menu-mobile:not(.sample)");
